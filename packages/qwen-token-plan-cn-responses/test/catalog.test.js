...
test("官方目录按个人版与 Responses 交集组装，并独立读取工具表", () => {
  ...
  assert.deepEqual(catalog.models.map((model) => model.id), ["qwen3.8-max", "qwen3.8-flash", "qwen3.7-max", "deepseek-v4-pro-0813"]);
  assert.equal(catalog.models[0].harnessTools.length, 5);
  assert.equal(catalog.models[3].harnessTools.length, 0);
  ...
  assert.deepEqual(catalog.models[3].reasoningEfforts, ["low", "high", "max"]);
  assert.equal(catalog.models[3].defaultReasoningEffort, "high");
});

test("探测覆盖只发布 API 已接受的推理档位", () => {
  const catalog = parseOfficialCatalog(DOCS, "2026-08-21T00:00:00.000Z", probes);
  const probed = catalog.models.find((model) => model.id === "deepseek-v4-pro-0813");
  assert.deepEqual(probed.reasoningEfforts, ["none", "low", "high", "max"]);
  assert.ok(probed.reasoningEfforts.every((effort) => probes.models["deepseek-v4-pro-0813"].accepted.includes(effort)));
});
